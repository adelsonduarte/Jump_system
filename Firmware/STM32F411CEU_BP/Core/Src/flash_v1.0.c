...
void FLASH_apaga(uint32_t endereco, uint8_t paginas){
	...
	//
	EraseInitStruct.TypeErase = FLASH_TYPEERASE_SECTORS;  // definição padrão da biblioteca HAL
	EraseInitStruct.Sector = endereco;				// definição do usuário do endereço da página que deseja apagar
	EraseInitStruct.NbSectors = paginas;					// número de páginas que serão apagadas
	//
